			...
			try {	
				...
				for(int r = 0; r < soru.size; r++)
				{
					for(int c = 0; c < soru.size; c++)
					{
						...
						thing = b.get(r).get(c);
						if(this.c == 1) {
							if(b.get(r).get(c) == 0) {g.setColor(c1);}
							else if(b.get(r).get(c) == 1) {g.setColor(c2);}
							else if(b.get(r).get(c) == 2) {g.setColor(c3);}
							g.fillRect(right_board+(r*hucreBoyu), up_space+(c*hucreBoyu), hucreBoyu, hucreBoyu);
						}
						else if(this.c == 2) {
							g.drawString(""+soru.getGameBoard().get(r).get(c), right_board+(int)(hucreBoyu*0.38)+(c*hucreBoyu), up_space+(int)(hucreBoyu*0.7)+(r*hucreBoyu));
						}
						else {
							g.drawString(""+lts[thing], right_board+(int)(hucreBoyu*0.38)+(c*hucreBoyu), up_space+(int)(hucreBoyu*0.7)+(r*hucreBoyu));
						}
						if(soru.question.get(r).get(c) != -1) {
							if(this.c == 1) {
								...
								g.fillRect(left_space+(r*hucreBoyu), up_space+(c*hucreBoyu), hucreBoyu, hucreBoyu);
							}
							else if(this.c == 2) {
								g.drawString(""+soru.getGameBoard().get(r).get(c), left_space+(int)(hucreBoyu*0.38)+(c*hucreBoyu), up_space+(int)(hucreBoyu*0.7)+(r*hucreBoyu));
							}
							else if(this.c == 0){
								g.drawString(""+lts[thing], left_space+(int)(hucreBoyu*0.38)+(c*hucreBoyu), up_space+(int)(hucreBoyu*0.7)+(r*hucreBoyu));
							}
						}
					}
				}
