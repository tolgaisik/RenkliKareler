...
public class Kareler {
    ...
    ArrayList<ArrayList<Integer>> possible,result,question;
    Kareler(int size, int count) {
        this.size = size;
        this.typeCount = count == 0 ? 2 : count;
        this.stop = false;
        this.board = new ArrayList<ArrayList<Integer>>();
        this.possible = new ArrayList<ArrayList<Integer>>(this.size); 
        this.result = new ArrayList<ArrayList<Integer>>(this.size);
        this.question = new ArrayList<ArrayList<Integer>>(this.size);
        this.initialize();
        this.getQ3();
        //System.out.println("End  "+this.count(this.question));
    }
    ...
    private void solvePartial(ArrayList<ArrayList<Integer>> list) {
        int str;
    ...
    private void getQ3() {
        ...
        while(true) {
            ...
            this.result.clear();
            for (ArrayList<Integer> arrayList : this.question) {
                this.result.add(new ArrayList<Integer>(arrayList));
            }
        }
    ...
    private int count(ArrayList<ArrayList<Integer>> list) {
        int counter = 0;
    ...
    //row and col has two different things 
    private void constrainsOne() {
        int rcounttwos=0,rcountones=0,rcountzeros=0,counttwos=0,countones=0,countzeros=0,cell,temp,temp2;
        int difference = this.size/this.typeCount;
        for (int i = 0; i < this.size; i++) {
            counttwos=0;countones=0;countzeros=0;
            for (int j = 0; j < this.size; j++) {
                cell = this.result.get(i).get(j);
                if(cell == -1) {
                    rcounttwos=0;rcountones=0;rcountzeros=0;counttwos=0;countones=0;countzeros=0;
                    ...
                    //0
                    if((rcountones == difference && counttwos == difference) || (countones == difference && rcounttwos == difference)) {
                        this.result.get(i).set(j, 0);
                    }
                    //1
                    else if((rcountzeros == difference && counttwos == difference) || (countzeros == difference && rcounttwos == difference)) {
                        this.result.get(i).set(j, 1);
                    }
                    //2
                    else if((rcountzeros == difference && countones == difference) || (countzeros == difference && rcountones == difference)) {
    ...
    private void solveRows() {
        ...
        for (int i = 0; i < this.size; i++) {
            countzeros = 0;counterones = 0;countertwos = 0;
            for(int j = 0; j < this.size; j++) {
                if(this.result.get(i).get(j) == 0){countzeros++;}
                else if(this.result.get(i).get(j) == 1){counterones++;}
                else if(this.result.get(i).get(j) == 2){countertwos++;}
            }
            if((counterones == typ && countzeros == typ) || (countertwos == typ && counterones == typ) || (countertwos == typ && countzeros == typ)) {
                for (int j = 0; j < this.size; j++) {
                    this.result.get(i).set(j, this.possible.get(i).get(j));
                }
            }
    ...
    private void solveCols() {
        ...
        for (int i = 0; i < this.size; i++) {
            countzeros = 0;counterones = 0;countertwos = 0;
            for(int j = 0; j < this.size; j++) {
                if(this.result.get(j).get(i) == 0){countzeros++;}
                else if(this.result.get(j).get(i) == 1){counterones++;}
                else if(this.result.get(j).get(i) == 2){countertwos++;}
            }
            if((counterones == typ && countzeros == typ) || (countertwos == typ && counterones == typ) || (countertwos == typ && countzeros == typ)) {
                for (int j = 0; j < this.size; j++) {
                    this.result.get(j).set(i, this.possible.get(j).get(i));
    ...
    private boolean isSolved(ArrayList<ArrayList<Integer>> list) {
        for (ArrayList<Integer> arrayList : list) {
